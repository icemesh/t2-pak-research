...
//add       r0, r0, r9
		...
		* old code
		uint32_t counter = 0;
		do{
			uint32_t numloginPageEntries = ReadU32(&pLoginPage->m_numLoginPageEntries);
			if (numloginPageEntries != 0)
			{
				uint32_t offset = ReadU32(&pLoginPage->m_dataOffset);
				...
				if (numloginPageEntries > 0)
				{
					...
					do {
						//lhz       r9, 0(r7)
						uint16_t page1Idx = ReadU16(&pEntry->m_page1Idx);
						//lhz       r11, 2(r7)
						uint16_t page2Idx = ReadU16(&pEntry->m_page2Idx);
						//lwz       r10, 4(r7)
						uint32_t unkData = ReadU32(&pEntry->m_offset);
						//lwzx      r8, r5, r9
						uint8_t* pPage1 = reinterpret_cast<uint8_t*>(m_pLoadedFile) + m_pageHdrs[page1Idx];
						//lwzx      r9, r5, r11
						uint8_t* pPage2 = reinterpret_cast<uint8_t*>(m_pLoadedFile) + m_pageHdrs[page2Idx];
...
{
	StringId64 typeId = SID(reinterpret_cast<const char*>(pResItem->m_itemTypeOffset));
	switch (typeId)
	{
